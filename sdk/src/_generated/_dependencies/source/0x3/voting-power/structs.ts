...
export function isVotingPowerInfo(type: string): boolean {
  ...
  return type === `${PKG_V16}::voting_power::VotingPowerInfo`;
}
...
export class VotingPowerInfo implements StructClass {
  ...
  static readonly $typeName = `${PKG_V16}::voting_power::VotingPowerInfo`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = VotingPowerInfo.$typeName;
  readonly $fullTypeName: `${typeof PKG_V16}::voting_power::VotingPowerInfo`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: VotingPowerInfoFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V16}::voting_power::VotingPowerInfo`;
...
export function isVotingPowerInfoV2(type: string): boolean {
  type = compressSuiType(type);
  return type === `${PKG_V16}::voting_power::VotingPowerInfoV2`;
}
...
export type VotingPowerInfoV2Reified = Reified<
  VotingPowerInfoV2,
  VotingPowerInfoV2Fields
>;

export class VotingPowerInfoV2 implements StructClass {
  __StructClass = true as const;

  static readonly $typeName = `${PKG_V16}::voting_power::VotingPowerInfoV2`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = VotingPowerInfoV2.$typeName;
  readonly $fullTypeName: `${typeof PKG_V16}::voting_power::VotingPowerInfoV2`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: VotingPowerInfoV2Fields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V16}::voting_power::VotingPowerInfoV2`;
    this.$typeArgs = typeArgs;
  ...
  static reified(): VotingPowerInfoV2Reified {
    return {
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        VotingPowerInfoV2.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        VotingPowerInfoV2.fromSuiObjectData(content),
      fetch: async (client: SuiClient, id: string) =>
        VotingPowerInfoV2.fetch(client, id),
  ...
  static fromSuiParsedData(content: SuiParsedData): VotingPowerInfoV2 {
    ...
    if (!isVotingPowerInfoV2(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a VotingPowerInfoV2 object`,
      );
    }
  ...
  static fromSuiObjectData(data: SuiObjectData): VotingPowerInfoV2 {
    if (data.bcs) {
      if (
        data.bcs.dataType !== "moveObject" ||
        !isVotingPowerInfoV2(data.bcs.type)
      ) {
  ...
  static async fetch(
    ...
    if (res.error) {
      throw new Error(
        `error fetching VotingPowerInfoV2 object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isVotingPowerInfoV2(res.data.bcs.type)
    ) {
